...
function rmSettings (name) {	// IN PROGRESS
	...
	this.button        = new RemoteControlBasic(name);          // rm_remotes-elements.js
	...
	this.toggle        = new RemoteElementSlider(name+".toggle");       // rm_remotes-slider.js
	this.index_buttons = undefined;
    this.elements      = new RemoteSettingsElements(name+".elements", this);
    ...
    // create container and load data with system information
    ...
    this.module_timer_select    = function (key, target, selected) {
        ...
        console.error(value);
        console.error(data);
        var basic           = new RemoteElementsEdit(name+".basic")
    ...
    this.module_interface_edit_info = function (data) {
        ...
        console.debug(this.app_name+".module_interface_edit_info:");
        console.debug(data);
        ...
        this.button_add_device = function(api, api_device, external_id) {
            ...
            if (!dataAll["CONFIG"]["apis"]["list_detect"][api+"_"+api_device]) {
                this.logging.warn("No detected devices found for "+api+"_"+api_device+".");
                return "";
            }
            const devices_detect = dataAll["CONFIG"]["apis"]["list_detect"][api+"_"+api_device];
            let html = "";
        ...
        this.list_api_device_settings = function (interface, device, show_buttons=undefined) {
            ...
            if (dataAll["CONFIG"]["apis"]["list_api_commands"][interface+"_"+device] && dataAll["CONFIG"]["apis"]["list_api_commands"][interface+"_"+device].length > 0) {
                ...
                for (var i=0;i<dataAll["CONFIG"]["apis"]["list_api_commands"][interface+"_"+device].length > 0;i++) {
                    ...
                    buttons_plus += this.button.sized("api_cmd_"+interface+"_"+device, command, "settings", command_link);
                    }
                }
            ...
            if (interfaces[interface]["API-Devices"][device]["AdminURL"]) {
                ...
                buttons_admin += this.button.sized("api_cmd_"+interface+"_"+device+"_admin", "open", "settings", cmd_url);
            }
        ...
        this.create_device_configuration = function (interface, device) {
            ...
            if (detect_devices) {
                ...
                Object.keys(detected).forEach(key => {
                    let api_string = interface + "_" + device + "||";
            ...
            if (config_create) {
                ...
                temp += this.tab.end();
                temp += this.button.edit("apiSendToDeviceApi( getValueById('api_device-"+interface+"_"+device+"'), getTextById('api_command-"+interface+"_"+device+"'), true);"+activate_copy_button, lang("CREATE"), "disabled", "create_button_"+interface+"_"+device) + "&nbsp;";
                temp += this.button.edit("copyTextById('JSON_copy',appMsg,'"+lang("COPIED_TO_CLIPBOARD")+"');"+activate_copy_button, lang("COPY"), "disabled", "copy_button_"+interface+"_"+device);
            }
            return temp;
        }
    ...
    // create container and load data for general settings
    ...
    // create container and load data for macro settings
	this.module_macros_edit     = function () {
		...
		setting  += this.button.sized(id="add_scene",label=lang("BUTTON_T_SAVE"),style="settings","apiMacroChange([#groups#,#macro#,#dev-on#,#dev-off#]);","");
    ...
    // create dialogs to add devices, scenes and remotes
    ...
    this.modules_add_remote_dialog = function(device_data_start={}) {
        ...
        set_temp += "<center>";
        set_temp += this.button.sized(id="add_dev",label="Add Device",style="settings",add_command);
        set_temp += "</center>";
        return set_temp;
		}
	...
	this.device_list_status	    = function (id_filter, id_list_container) {
		...
		for (var key in device_status) {
			if (key == "power") {
				command_on    = "appFW.requestAPI('GET',['set','"+filter+"','"+key+"','ON'], '', '', '' );rm3settings.on_off();remoteInit();";
				command_off   = "appFW.requestAPI('GET',['set','"+filter+"','"+key+"','OFF'], '', '', '' );rm3settings.on_off();remoteInit();";
...
class RemoteSettingsElements {
    ...
    constructor(app_name, parent) {
        ...
        this.input_width = parent.input_width;
    }
    ...
    input (id, onclick="", oninput="", value="") {
        ...
        let text = "<input id=\"" + id + "\" oninput=\""+oninput+"\" style='width:" + this.input_width + ";margin:1px;' value='" + value + "'>";
        if (onclick !== "") {
